...
As announced in [May 2024](https://github.com/livesession/oapi-codegen/discussions/1605),
...
`github.com/deepmap/oapi-codegen/v2` to `github.com/livesession/oapi-codegen/v2`.
...
```sh
# for the binary install
go install github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen@latest
```
...
```sh
$ go get -tool github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen@latest
...
import (
	_ "github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen"
)
```
...
```go
//go:generate go run github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen --config=config.yaml ../../api.yaml
```
...
While the project does not ([yet](https://github.com/livesession/oapi-codegen/issues/1519)) have a defined release cadence, there may be cases where you want to pull in yet-unreleased changes to your codebase.
...
$ go get github.com/livesession/oapi-codegen/v2@main
# alternatively, to a commit hash i.e. https://github.com/livesession/oapi-codegen/commit/71e916c59688a6379b5774dfe5904ec222b9a537
$ go get github.com/livesession/oapi-codegen/v2@71e916c59688a6379b5774dfe5904ec222b9a537
```
...
+++ go.mod
@@ -2,21 +2,20 @@
-require github.com/livesession/oapi-codegen/v2 v2.1.0
+require github.com/livesession/oapi-codegen/v2 v2.1.1-0.20240331212514-80f0b978ef16
```
...
For full details of what is supported, it's worth checking out [the GoDoc for `codegen.Configuration`](https://pkg.go.dev/github.com/livesession/oapi-codegen/v2/pkg/codegen#Configuration).
...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/livesession/oapi-codegen/HEAD/configuration-schema.json
package: api
# ...
```
...
In this case, we will expose a [compatibility option](https://pkg.go.dev/github.com/livesession/oapi-codegen/v2/pkg/codegen#CompatibilityOptions) to restore old behaviour.
...
- Support of OpenAPI 3.0
  - OpenAPI 3.1 support is [awaiting upstream support](https://github.com/livesession/oapi-codegen/issues/373)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/stdhttp/api"
)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/chi/api"
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/gorillamux/api"
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/echo/api"
	"github.com/labstack/echo/v4"
)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/fiber/api"
	"github.com/gofiber/fiber/v2"
)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/gin/api"
	"github.com/gin-gonic/gin"
)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/minimal-server/iris/api"
	"github.com/kataras/iris/v12"
)
...
//go:generate go run github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen -config cfg.yaml api.yaml
```
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/examples/client"
)
...
```
# github.com/livesession/oapi-codegen/v2/examples/clienttypenameclash
        ...
        400:
          content:
            application/json:
              ...
              # See https://github.com/livesession/oapi-codegen/issues/1512
              schema:
                type: object
                properties:
                  code:
...
```sh
$ go run github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen -config cfg-api.yaml ../admin/api.yaml
$ go run github.com/livesession/oapi-codegen/v2/cmd/oapi-codegen -config cfg-user.yaml ../common/api.yaml
```
...
import-mapping:
  ...
  ../common/api.yaml: github.com/livesession/oapi-codegen/v2/examples/import-mapping/common
```
...
import (
	// ...
	externalRef0 "github.com/livesession/oapi-codegen/v2/examples/import-mapping/common"
)
...
As of `oapi-codegen` [v2.1.0](https://github.com/livesession/oapi-codegen/releases/tag/v2.1.0) it is now possible to represent this with the `nullable.Nullable` type from [our new library, oapi-codegen/nullable](https://github.com/oapi-codegen/nullable).
...
> [!NOTE]
> It is [not currently possible](https://github.com/livesession/oapi-codegen/issues/1038) to validate the HTTP response with a middleware.
...
>
> In the future, we plan to [implement server-side validation in the generated code](https://github.com/livesession/oapi-codegen/issues/1524)
...
import (
	...
	"github.com/livesession/oapi-codegen/v2/pkg/securityprovider"
)
...
Notice that we're using a pre-built provider from the [`pkg/securityprovider` package](https://pkg.go.dev/github.com/livesession/oapi-codegen/v2/pkg/securityprovider), which has some inbuilt support for other types of authentication, too.
...
>
> This will be disabled by default (but possible to turn back on via configuration) [in the future](https://github.com/livesession/oapi-codegen/issues/1564)
...
output-options:
  user-templates:
    ...
    client-with-responses.tmpl: https://raw.githubusercontent.com/livesession/oapi-codegen/ad5eada4f3ccc28a88477cef62ea21c17fc8aa01/pkg/codegen/templates/client-with-responses.tmpl
    # Alternatively using raw.githubusercontent.com with a tag
    client-with-responses.tmpl: https://raw.githubusercontent.com/livesession/oapi-codegen/v2.1.0/pkg/codegen/templates/client-with-responses.tmpl
    # Alternatively using raw.githubusercontent.com with a branch
    client-with-responses.tmpl: https://raw.githubusercontent.com/livesession/oapi-codegen/master/pkg/codegen/templates/client-with-responses.tmpl
```
...
Alternatively, you are able to use the underlying code generation as a package, which [will be documented in the future](https://github.com/livesession/oapi-codegen/issues/1487).
...
> [!NOTE]
> In the future [this will be possible](https://github.com/livesession/oapi-codegen/issues/1514) to disable this functionality, and honour the implicit `additionalProperties: true`
...
components:
  schemas:
    ...
    # https://github.com/livesession/oapi-codegen/issues/1569
...
Check [the docs](https://pkg.go.dev/github.com/livesession/oapi-codegen/v2/pkg/codegen#OutputOptions) for more details of usage.
...
We're very appreciative of [the many contributors over the years](https://github.com/livesession/oapi-codegen/graphs/contributors) and the ongoing use of the project 💜

<a href="https://github.com/livesession/oapi-codegen/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=livesession/oapi-codegen" />
</a>

## Sponsors

For the most part, `oapi-codegen` is maintained in two busy peoples' free time. As noted in [Creating a more sustainable model for `oapi-codegen` in the future](https://github.com/livesession/oapi-codegen/discussions/1606), we're looking to make this a more sustainable project in the future.
