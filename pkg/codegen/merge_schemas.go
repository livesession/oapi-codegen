...
func equalTypes(t1 *openapi3.Types, t2 *openapi3.Types) bool {
	...
	// NOTE that ideally we'd use `slices.Equal` but as we're currently supporting Go 1.20+, we can't use it (yet https://github.com/livesession/oapi-codegen/issues/1634)
	for i := range s1 {
		if s1[i] != s2[i] {
			return false
		}
